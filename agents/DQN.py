...
class Agent(BaseAgent):
    ...
    def declare_networks(self):
        if self.config.dueling_dqn:
            self.q_net = DuelingDQN(self.num_feats, self.num_actions, noisy=self.config.noisy_nets, sigma_init=self.config.sigma_init, body=AtariBody)
            self.target_q_net = DuelingDQN(self.num_feats, self.num_actions, noisy=self.config.noisy_nets, sigma_init=self.config.sigma_init, body=AtariBody)
        else:
            self.q_net = DQN(self.num_feats, self.num_actions, noisy=self.config.noisy_nets, sigma_init=self.config.sigma_init, body=AtariBody)
            self.target_q_net = DQN(self.num_feats, self.num_actions, noisy=self.config.noisy_nets, sigma_init=self.config.sigma_init, body=AtariBody)
        ...
        with torch.no_grad():
            ...
            if not empty_next_state_values:
                if self.config.double_dqn:
                    max_next_actions = torch.argmax(self.q_net(non_final_next_states), dim=1).view(-1, 1)
                    next_q_values[non_final_mask] = (self.config.gamma**self.config.N_steps) * self.target_q_net(non_final_next_states).gather(1, max_next_actions)
                else:
                    next_q_values[non_final_mask] = (self.config.gamma**self.config.N_steps) * self.target_q_net(non_final_next_states).max(dim=1)[0].view(-1, 1)
