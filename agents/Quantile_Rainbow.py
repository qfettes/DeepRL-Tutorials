...
class Agent(DQN_Agent):
    ...
    def declare_networks(self):
        self.q_net = DuelingQRDQN(self.env.observation_space.shape, self.env.action_space.n,
                                  noisy_nets=True, noisy_sigma=self.noisy_sigma, quantiles=self.num_quantiles)
        self.target_q_net = DuelingQRDQN(self.env.observation_space.shape, self.env.action_space.n,
                                         noisy_nets=True, noisy_sigma=self.noisy_sigma, quantiles=self.num_quantiles)

    def declare_memory(self):
        self.memory = PrioritizedReplayMemory(
            self.experience_replay_size, self.priority_alpha, self.priority_beta_start, self.priority_beta_frames)
    ...
    def next_distribution(self, batch_vars):
        ...
        with torch.no_grad():
            ...
            quantiles_next = batch_reward + \
                ((self.gamma**self.nsteps)*quantiles_next)
    ...
    def compute_loss(self, batch_vars):
        ...
        loss = self.huber(
            diff) * torch.abs(self.cumulative_density.view(1, -1) - (diff < 0).to(torch.float))
    ...
    def get_action(self, s, eps):
        with torch.no_grad():
            ...
            self.q_net.sample_noise()
            a = (self.q_net(X) * self.quantile_weight).sum(dim=2).max(dim=1)[1]
            return a.item()

    def get_max_next_state_action(self, next_states):
        next_dist = self.q_net(next_states) * self.quantile_weight
