...
class AtariBodyAC(nn.Module):
    def __init__(self, input_shape, body_out=64, noisy_nets=False, sigma_init=0.4):
        ...
        self.fc1 = init_(nn.Linear(self.feature_size(input_shape), body_out)) if not self.noisy_nets else init_(NoisyLinear(self.feature_size(input_shape), body_out, sigma_init))
...
class SimpleBodyAC(nn.Module):
    def __init__(self, input_shape, output_shape=200, noisy_nets=False, sigma_init=0.5):
        ...
        self.fc1 = init_(nn.Linear(input_shape[0], output_shape)) if not self.noisy_nets else init_(NoisyLinear(input_shape[0], output_shape, sigma_init))
