...
class ActorCritic(nn.Module):
    def __init__(self, input_shape, num_actions, conv_out=64, use_gru=False, gru_size=256, noisy_nets=False, sigma_init=0.5):
        ...
        if type(num_actions) is int:
            self.body = AtariBodyAC(input_shape, conv_out)
            num_outputs = num_actions
        elif num_actions.__class__.__name__ == 'Box':
            self.body = SimpleBodyAC(input_shape, conv_out, noisy_nets, sigma_init)
            num_outputs = num_actions.shape[0]
            self.logstd = nn.Parameter(torch.zeros(num_outputs))
        else:
            print('[ERROR] Incompatible action space for ActorCritic NN')
            exit()
        ...
        if use_gru:
            self.fc1 = init_(nn.Linear(self.body.feature_size(input_shape), 512)) if not self.noisy_nets else init_(NoisyLinear(self.body.feature_size(input_shape), 512, sigma_init))
            ...
            self.fc1 = init_(nn.Linear(self.body.feature_size(input_shape), self.gru_size)) if not self.noisy_nets else init_(NoisyLinear(self.body.feature_size(input_shape), self.gru_size, sigma_init))
        ...
        self.actor_linear = init_(nn.Linear(self.gru_size, num_outputs)) if not self.noisy_nets else init_(NoisyLinear(self.gru_size, num_outputs, sigma_init))
...
class ContinuousActorCritic(nn.Module):
    def __init__(self, input_shape, num_actions, body_out=64, use_gru=False, gru_size=256, noisy_nets=False, sigma_init=0.5):
        ...
        #                 noisy_layer=self.noisy_nets)
        # if use_gru:
        #     self.gru = nn.GRU(self.body.feature_size(input_shape), self.gru_size)
        #     for name, param in self.gru.named_parameters():
        #         if 'bias' in name:
        #             nn.init.constant_(param, 0)
        #         elif 'weight' in name:
        ...
        self.critic_linear = init_(nn.Linear(self.body_out, 1)) if not self.noisy_nets else init_(NoisyLinear(self.gru_size, 1, sigma_init))

        init_ = lambda m: self.layer_init(m, nn.init.orthogonal_,
                    lambda x: nn.init.constant_(x, 0), gain=0.01,
                    noisy_layer=self.noisy_nets)

        self.actor_linear = init_(nn.Linear(self.body_out, num_outputs)) if not self.noisy_nets else init_(NoisyLinear(self.gru_size, num_outputs, sigma_init))
    ...
    def forward(self, inputs, states, masks):
        x_actor, x_critic = self.body(inputs)
