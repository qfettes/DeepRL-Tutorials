...
class RolloutStorage(object):
    def __init__(self, num_steps, num_processes, obs_shape, action_space, state_size, device, USE_GAE=True, gae_tau=0.95, use_proper_time_limits=True):
        self.observations = torch.zeros(
            num_steps + 1, num_processes, *obs_shape).to(device)
        self.states = torch.zeros(
            num_steps + 1, num_processes, state_size).to(device)
        ...
        self.actions = torch.zeros(
            num_steps, num_processes, actions_shape).to(device)
        if action_space.__class__.__name__ == 'Discrete':
            self.actions = self.actions.to(torch.long)
        else:
            self.actions = self.actions.to(torch.float)
    ...
    def insert(self, current_obs, state, action, action_log_prob, value_pred, reward, mask, bad_masks):
        ...
        if self.use_proper_time_limits:
            if self.gae:
                ...
                for step in reversed(range(self.rewards.size(0))):
                    ...
                    gae = delta + gamma * self.gae_tau * self.masks[step +
                                                                    1] * gae
    ...
    def feed_forward_generator(self, advantages, num_mini_batch):
        ...
        for indices in sampler:
            ...
            states_batch = self.states[:-
                                       1].view(-1, self.states.size(-1))[indices]
