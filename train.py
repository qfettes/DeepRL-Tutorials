...
# TODO: categorical needs tuning to original paper
# TODO: priority replay in c51 might use incorrect priority
# TODO: add tensorboard logging for state and next_state v estimates to c51
# TODO: Noisy net efficiency issue?
...
from utils.wrappers import make_env_general
...
def train(config, Agent, ipynb=False):
    ...
    envs = make_env_general(config.env_id, config.seed, log_dir, config.num_envs, config.gamma, stack_frames=config.stack_frames, adaptive_repeat=config.adaptive_repeat, sticky_actions=config.sticky_actions, clip_rewards=True)
